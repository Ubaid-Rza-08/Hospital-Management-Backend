...
public class AuthUtil {
    ...
    public String determineUsernameFromOAuth2User(OAuth2User oAuth2User, String registrationId, String providerId) {
        ...
        if (email != null && !email.isBlank()) {
            // You can still choose to keep username != email; this returns email for convenience
            return email;
        }
        return switch (registrationId.toLowerCase()) {
            case "google" -> oAuth2User.getAttribute("sub"); // Google subject
            case "github" -> oAuth2User.getAttribute("login"); // GitHub handle
            case "facebook" -> oAuth2User.getAttribute("id"); // Fallback to id
            default -> providerId;
        };
    }
}
